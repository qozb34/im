...
def adds2(session: str, phone: str, user_id: int)-> bool:
    d = db.get(f'ses_{user_id}') if db.exists(f'ses_{user_id}') else []
    d.append({"s":session, 'phone': phone})
    db.set(f'ses_{user_id}', d)
    return True
try:
...
app = Client('::memory::', in_memory=True, api_hash='a0c224dc0c362b07b450f61b8689977b', api_id=20130471, lang_code="ar", bot_token=w['give_bot_token'])
...
async def clear(app, call):
    ...
    for session in sessions:
        ...
        phon = session['phone']
        try:
            client = temp('::memory::', api_id=25453029, api_hash='ed66bc9eba4e8d21d0041b257a1e525a', in_memory=True, session_string=sessio)
...
@app.on_message(filters.command(['add_private']))
async def ec(app, message):
    await generate_session2(app, message)
    
async def generate_session2(app, message):
    password = None 
    phone = None
    code = None
    msg = message
    ...
    api_id = 21196718
    api_hash = "0f3731e591c2dfd8f16b18ee7637eee9"
    prem = db.get(f'private_{user_id}')
    if not prem:
        await message.reply("• عذرا ، هذا القسم متاح فقط للاعضاء الـ vip")
        return
    ask = await app.ask(message.chat.id,"• ارسل الان رقم الهاتف الخاص بك مع رمز الدولة ليتم اضافته في قائمة حساباتك\n• مثال: \n+20466133155")
    if ask.text == "/start":
    	await ec(app,message)
    	return
    if "+" not in ask.text:
